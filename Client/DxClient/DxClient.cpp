...
void ColoredWavesDemo::TestGRPFile()
{
	//CGrpFile grp;
	//grp.Open("00023.grp");

	DWORD filebuffer[256*256];
	
// 	CFileStream fStream;
// 	fStream.Open("../Client/nnn-0.nnn");
// 	fStream.Read(filebuffer, 256*256*4);

	RECT rc = {0, 0,64, 64};
	IDirect3DSurface9* pSurface;
	IDirect3DTexture9* pTexture;
	//HRESULT hr = gd3dDevice->CreateTexture(64,64,1, 0, D3DFMT_DXT3, D3DPOOL_SYSTEMMEM, &pTexture, 0);
	HRESULT hr = gd3dDevice->CreateOffscreenPlainSurface(64,64,D3DFMT_DXT3, D3DPOOL_SYSTEMMEM, &pSurface, 0);
	if(SUCCEEDED(hr) && pSurface)
	{
		//pTexture->GetSurfaceLevel(0, &pSurface);
		if(pSurface)
		{
			DWORD dwBuffer[64*64];
			for(DWORD dwX = 0; dwX < 64*64; ++dwX)
			{
				if(dwX % 4 == 0)
					dwBuffer[dwX] = 0xFF000000;
				else
					dwBuffer[dwX] = 0xFFFFFFFF;
			}
			D3DXLoadSurfaceFromMemory(pSurface, NULL, &rc, dwBuffer, D3DFMT_A8B8G8R8, 64*4, NULL, &rc, D3DX_FILTER_NONE, 0x00ffffff);

			D3DXSaveSurfaceToFile("ltest_dxt3.tga", D3DXIFF_TGA, pSurface, NULL, &rc);
		}
	}
